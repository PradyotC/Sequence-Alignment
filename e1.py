...
class SeqAlignEff:
	...
	def fillDataDyn(self,str1,str2):
		...
		self.algnArr = [[0 for _ in range(n)] for _ in range(m)]
		for i in range(m):
			for j in range(n):
				if i == 0:
	...
	def getAlignDyn(self,str1,str2):
		...
		while True:
			if m ==0 and n == 0:
				...
				a = self.alpha(str1[m-1],str2[n-1]) + self.algnArr[m-1][n-1]
				...
				if mVal == a:
					...
					rstr2 += '_'
					m-= 1
		return self.algnArr[-1][-1],rstr1[::-1], rstr2[::-1]

	def fillData(self,str1,str2):
		str1, str2 = 'A'+ str1,'A'+ str2
		m, n = len(str1), len(str2)
		self.algnArr = [[0,self.dlt*i] for i in range(n)]
		for j in range(1,m):
			for i in range(n):
				self.algnArr[i] = [self.algnArr[i][1],0]
			self.algnArr[0][1] = self.dlt*j
			for i in range(1,n):
				c1,c2 = str1[j],str2[i]
				val = min(self.alpha(c1,c2)+self.algnArr[i-1][0], self.dlt+self.algnArr[i-1][1], self.dlt+self.algnArr[i][0])
				self.algnArr[i][1] = val
		return [i[1] for i in self.algnArr]
